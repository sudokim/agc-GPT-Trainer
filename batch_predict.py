...
def predict(
    ...
    **kwargs,
    ...
    if "{model}" not in prediction_path:
        raise ValueError("prediction_path must contain {model}")
    ...
    for i, adapter_weight in enumerate(adapter_weight_path):
        ...
        fp = open(prediction_path.format(model=adapter_weight.name), "w", encoding="utf-8")
        ...
        fp.close()
        end_time = time.time()
        times[adapter_weight.name][i] = round(end_time - start_time, 2)
